...
#define LED_BLUE D7 //BLUE:D8
#define LED_GREEN D8 //GREEN:D7
...
String texto_humedad;
String texto_temperatura;
String texto_humedadsuelo;
...
void hallarestado_Temperatura(){
  ...
  if(temp>=0 && temp<=18){
    ...
    texto_temperatura="Temperatura: Frio";
  }
  //Si la temperatura es optima
  else if(temp>18 && temp<=30){
    estado_temperatura=optimo;
    texto_temperatura="Temperatura: Optima";
  }
  //Si la temperatura es caliente
  else if(temp>30 && temp<=50){
    estado_temperatura=caliente;
    texto_temperatura="Temperatura: Caliente";
  }
  else{
    estado_temperatura=0;
    texto_temperatura="Temperatura NA";
    }
}
...
void hallarestado_HumedadSuelo(){
  ...
  if(humedad_suelo>=0 && humedad_suelo<=20){
    estado_humedadsuelo=sueloseco;
    texto_humedadsuelo="Humedad del suelo: Suelo Seco";
  }
  //Si la humedad_suelo es humedo
  else if(humedad_suelo>20 && humedad_suelo<=45){
    estado_humedadsuelo=suelohumedo;
    texto_humedadsuelo="Humedad del suelo: Suelo Humedo";
  }
  //Si la humedad_suelo es muy humedo
  else if(humedad_suelo>45 && humedad_suelo<=100){
    estado_humedadsuelo=suelomuyhumedo;
    texto_humedadsuelo="Humedad del suelo: Suelo muy Humedo";
  }
   else{
    estado_humedadsuelo=0;
    texto_humedadsuelo="Huemdad del Suelo NA";
    }
}

void mostrarEstados(){
  Serial.print(texto_temperatura);
  Serial.print(" ; ");
  Serial.print(texto_humedad);
  Serial.print(" ; ");
  Serial.println(texto_humedadsuelo);
}
...
void MostrarUltrasonido(){
  ...
  if (primeraMedia)
  {
    ...
    Serial.print("Distancia lleno promedio: ");
    Serial.print(media);
    Serial.println(" cm");

    Serial.print("Cantidad liquido: ");
    Serial.print(cantidadLiquido);
    Serial.println(" ml");

    Serial.print("Porcentaje liquido: ");
...
//Condicional para porcentaje de liquido en el recipiente debe ser mayor a 25 para regar,caso contrario no hay liquido
//suficiente y debe apagarse la bomba de agua,el porcentaje depende del recipiente
  if(porcentaje>25){
      //Empezamos el riego
    ...
    apagar_riego();
    Serial.println("No es posible regar,llene liquido al recipiente....");
  }
